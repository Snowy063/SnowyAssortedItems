...
[Tracked(false)]
public class JuckterFieldRenderer : Entity
{
    private bool bubble;
    private List<JuckterField> list = new List<JuckterField>();
    ...
    public void Track(JuckterField block, Level level)
    {
        this.list.Add(block);
        if (this.tiles == null)
        {
            this.levelTileBounds = level.TileBounds;
    ...
    public void Untrack(JuckterField block)
    {
    ...
    private void RebuildEdges()
    {
        ...
        foreach (JuckterField parent in this.list)
        {
                ...
                for (int y = (int) parent.Y / 8; (double) y < (double) parent.Bottom / 8.0; ++y)
                {
                    foreach (Point point1 in pointArray)
                    {
                        ...
                        if (!this.Inside(x + point1.X, y + point1.Y) && (!this.Inside(x - point2.X, y - point2.Y) || this.Inside(x + point1.X - point2.X, y + point1.Y - point2.Y)))
                        {
                            ...
                            this.edges.Add(new JuckterFieldRenderer.Edge(parent, a, b));
                        }
                    }
                }
            }
    ...
    private void OnRenderBloom()
    {
        ...
        foreach (JuckterField JuckterField in this.list)
        {
            if (JuckterField.Visible)
                Draw.Rect(JuckterField.X, JuckterField.Y, JuckterField.Width, JuckterField.Height, Color.White);
        }
        foreach (JuckterFieldRenderer.Edge edge in this.edges)
        {
            if (edge.Visible)
            {
    ...
    public override void Render()
    {
        ...
        {
            if (JuckterField.Visible)
                Draw.Rect(JuckterField.Collider, color1);
        }
    ...
    private class Edge
    {
        ...
        public Edge(JuckterField parent, Vector2 a, Vector2 b)
        {
            this.Parent = parent;
            this.Visible = true;
            this.A = a;
